...
static GF256_LOG: [u8; 256] = [
    ...
    0x35, 0x93, 0xDA, 0x8E, 0x96, 0x8F, 0xDB, 0xBD, 0x36, 0xD0, 0xCE,
    0x94, 0x13, 0x5C, 0xD2, 0xF1, 0x40, 0x46, 0x83, 0x38, 0x66, 0xDD,
    0xFD, 0x30, 0xBF, 0x6, 0x8B, 0x62, 0xB3, 0x25, 0xE2, 0x98, 0x22, 0x88,
    0x91, 0x10, 0x7E, 0x6E, 0x48, 0xC3, 0xA3, 0xB6, 0x1E, 0x42, 0x3A,
    0x6B, 0x28, 0x54, 0xFA, 0x85, 0x3D, 0xBA, 0x2B, 0x79, 0xA, 0x15, 0x9B,
    0x9F, 0x5E, 0xCA, 0x4E, 0xD4, 0xAC, 0xE5, 0xF3, 0x73, 0xA7, 0x57,
    0xAF, 0x58, 0xA8, 0x50, 0xF4, 0xEA, 0xD6, 0x74, 0x4F, 0xAE, 0xE9,
    0xD5, 0xE7, 0xE6, 0xAD, 0xE8, 0x2C, 0xD7, 0x75, 0x7A, 0xEB, 0x16, 0xB,
    0xF5, 0x59, 0xCB, 0x5F, 0xB0, 0x9C, 0xA9, 0x51, 0xA0, 0x7F, 0xC, 0xF6,
    0x6F, 0x17, 0xC4, 0x49, 0xEC, 0xD8, 0x43, 0x1F, 0x2D, 0xA4, 0x76,
    0x7B, 0xB7, 0xCC, 0xBB, 0x3E, 0x5A, 0xFB, 0x60, 0xB1, 0x86, 0x3B,
    0x52, 0xA1, 0x6C, 0xAA, 0x55, 0x29, 0x9D, 0x97, 0xB2, 0x87, 0x90,
    0x61, 0xBE, 0xDC, 0xFC, 0xBC, 0x95, 0xCF, 0xCD, 0x37, 0x3F, 0x5B,
    0xD1, 0x53, 0x39, 0x84, 0x3C, 0x41, 0xA2, 0x6D, 0x47, 0x14, 0x2A,
    0x9E, 0x5D, 0x56, 0xF2, 0xD3, 0xAB, 0x44, 0x11, 0x92, 0xD9, 0x23,
...
fn fast_mul(a: u8, b: u8) -> u8 {
    if a == 0 || b == 0 {
        return 0u8;
    }
    let prod: u16 =
        GF256_LOG[a as usize] as u16 + GF256_LOG[b as usize] as u16;
    println!(
        "a: {:X} b: {:X} result:{:X} ",
        GF256_LOG[a as usize], GF256_LOG[b as usize], prod
    );

    if prod < 0xff {
        return GF256_EXP[prod as usize];
    }
    return GF256_EXP[(prod - 0xff) as usize];
}
...
fn generate_log() -> [u8; 256] {
    let mut log = [0_u8; 256];
    for i in 0..255usize {
        log[GF256_EXP[i as usize] as usize] = i as u8;
    }
    return log;
}

fn print_table(log: &[u8; 256]) {
    let column = 16;
    let mut counter = 0;
...
#[test]
fn fast_mul_works() {
    let res = fast_mul(0xb6, 0x53);
    assert_eq!(res, 0x36);
    // let res = fast_mul(0x1a, 0x53);
    // assert_eq!(res, 0x4f);
}
